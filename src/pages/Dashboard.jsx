...
export default function Dashboard() {
  ...
  // Trigger data fetching when switching active tabs
  useEffect(() => {
  ...
  // API Submit Handler: Market Item
  const handleCreateMarket = async (event) => {
    ...
    try {
      ...
      console.error('Failed to post market item:', error.message);
      alert(error.message || 'Failed to post item');
    }
  };

  // API Submit Handler: Campus Update
  const handleCreateUpdate = async (event) => {
    ...
    try {
      const payload = {
        ...
        author: user?.full_name || user?.name || 'Administration'
      };
  ...
  // API Submit Handler: Campus Event
  ...
  return (
    <div className="dashboard-layout">
      ...
      <main className="dashboard-main">
            ...
            <div className="module-grid">
              <div className="module-list">
                {marketItems.length === 0 ? (
                  <p className="no-data-msg">No market items published yet.</p>
                ) : (
                  marketItems.map((item) => (
                    <article key={item.id || item.item_id} className="listing-card market-card">
                      ...
                      <div className="seller-row">
                        ...
                        <span className={`role-badge ${(item.postedByRole || item.posted_by_role || 'user').toLowerCase()}`}>
                          {item.postedByRole || item.posted_by_role || 'USER'}
                        </span>
                      </div>
                    </article>
                  ))
            ...
            <div className="module-grid">
              <div className="module-list">
                {campusUpdates.length === 0 ? (
                  <p className="no-data-msg">No campus updates available.</p>
                ) : (
                  campusUpdates.map((item) => (
                    <article key={item.id || item.update_id} className="listing-card update-card">
                      <div className="listing-topline">
                      ...
                      <h4>{item.title}</h4>
                      <div className="meta-row">
                        <span>{item.audience || 'All students'}</span>
                        <span>{item.createdAt ? new Date(item.createdAt).toLocaleDateString() : item.created_at}</span>
                      </div>
                      <p className="description">{item.summary || item.content}</p>
                      <div className="seller-row">
                        <div>
                          <strong>{item.author || 'Administration'}</strong>
                        </div>
                        <span className={`role-badge ${(item.postedByRole || 'admin').toLowerCase()}`}>
              ...
              <div className="module-list">
                {campusEvents.length === 0 ? (
                  ...
                  campusEvents.map((item) => (
                    <article key={item.id || item.event_id} className="listing-card event-card">
                      <div className="listing-topline">
                      ...
                      <div className="seller-row">
                        <div>
                          <strong>{item.host || 'Campus Office'}</strong>
                        </div>
                        <span className={`role-badge ${(item.postedByRole || 'admin').toLowerCase()}`}>
                          {item.postedByRole || 'ADMIN'}
                        </span>
              ...
              {canManageCampusContent && (
                <form className="form-panel" onSubmit={handleCreateEvent}>
                  ...
                  <div className="field-grid">
                    ...
                    <input value={eventForm.host} onChange={(e) => setEventForm({ ...eventForm, host: e.target.value })} placeholder="Host / organizing office" required />
        ...
        {/* Other Tab Views */}
        {activeTab === 'hostels' && <div className="tab-view-container"><h3>Hostels</h3></div>}
        {activeTab === 'profile' && (
